...
def encode(request: dict[str, Any]) -> tuple[str, int, bytes]:
    ...
    if op == "register":
        ...
        header = struct.pack("<QB7xIIII", int(alive or 0), int(alive is not None), *sec)
        ...
        ids = b"".join(struct.pack("<Q", int(value)) for value in request.get("id", []))
        sec, payload = sections(robot, key, ids)
        ...
        sec, payload = sections(robot, key)
        header = struct.pack("<QIIII", int(request["id"]), *sec)
...
def decode_reply(message: peerbus.DatapodMessage) -> dict[str, Any]:
    ...
    blocked, decision, reason, has_blocked = struct.unpack("<QBBB5x", wire)
