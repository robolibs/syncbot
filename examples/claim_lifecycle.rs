...
fn main() {
    ...
    let r1 = ClaimRequest {
        ...
        targets: vec![ClaimTarget { kind: ClaimTargetKind::Zone, resource_id: zone_id }],
        ..ClaimRequest::default()
    };
    println!("first request — {:?}", mgr.evaluate_request(&r1).decision);
    mgr.add_request(r1.clone());
    mgr.add_lease(Lease {
        id: LeaseId::new(11), claim_id: ClaimId::new(1), robot_id: RobotId::new(1),
        access_mode: ClaimAccessMode::Exclusive,
        targets: r1.targets.clone(),
        granted_at_tick: Some(0), expires_at_tick: Some(100),
        ..Lease::default()
    });
