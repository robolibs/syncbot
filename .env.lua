-- syncbot's directory environment. Loaded when you `cd` here, unloaded when you leave.
...
-- The Python binding as `make bind-py` unpacks it, and the barn example, so
-- `python3 examples/python/barn/server.py` and friends run from anywhere.
oslo.direnv.path_add("./target/python", "PYTHONPATH")
oslo.direnv.path_add("./examples/python/barn", "PYTHONPATH")
