...
#align(center)[
  ...
  #v(0.4em)
  #text(size: 11pt, fill: muted)[The same battery, real commands: JSON, XML, ROS2, and mixed]
]

#v(1em)
#note[
  One protocol, several transports — every command shown in full. The
  register / heartbeat / claim / release battery runs over *REST/JSON* (`curl`),
  over *REST/XML* (`curl`), over *ROS2* (`ros2 service call` through
  `zenoh-bridge-ros2dds`), and *mixed*
  across all three, against one coordinator and one 7-zone workspace
...
= Mixed (REST/JSON + REST/XML + ROS2, one coordinator)

Three robots on *different transports* against the *same* server: robot `91`
speaks REST/XML (`curl`), robot `92` speaks ROS2 (`ros2 service call`), and
robot `93` speaks REST/JSON (`curl`). We register one on each, then run the
experiments — a claim on any of them blocks the others, in every direction, and
a release on one frees the zone for the rest.
...
# --- experiment 4: JSON robot 93 takes zone 2, both others are blocked ---
curl -s -X POST "$B/claims/zone" -H 'content-type: application/json' \
...
ros2 service call /ares/v1/claims/zone ares_interfaces/srv/Json \
  "{request: '{\"key\":\"2222\",\"robot\":\"92\",\"id\":[2]}'}"
# -> response: {"decision":0,"reason":2,"blocked":2}                  (blocked by JSON 93)
...
#note[Captured live: REST/JSON and REST/XML via `curl`, ROS2 via
`ros2 service call` through `zenoh-bridge-ros2dds` on CycloneDDS, all against
one coordinator on the same seven-zone workspace.]
