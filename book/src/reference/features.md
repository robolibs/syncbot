...
| `quicbit`          | `quicbit`           | [quicbit](../wire/quicbit.md) publisher              |
...
The `wire` module is compiled when any of `rest`, `xmlt`, `robo`, or `quicbit` is
on. The shared types and `ServeState` live there regardless of which adapter you
pick.
...
cargo check --features quicbit           # quicbit
cargo check --features "rest robo xmlt"  # all HTTP/Zenoh transports
```
...
- `quicbit` builds iceoryx2 (needs `libclang`) and iroh; heavier, fully isolated
  behind the feature.
